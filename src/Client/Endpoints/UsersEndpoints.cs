...
public static class UsersEndpoints
{
    ...
    private static void MapEndpoints(RouteGroupBuilder g)
    {
        ...
        g.MapGet(
            "all-users",
            async (int? pageNumber, int? pageSize, [FromServices] ISender sender, CancellationToken cancellationToken)=>
            {
        ...
        g.MapPut(
                "",
                async (string userId, string? newPhoneNumber, string? newName, string? newEmail, [FromServices] ISender sender, CancellationToken cancellationToken) =>
                {
                    await sender.Send(new ChangeUserCommand(){UserId = userId, NewEmail = newEmail, NewName = newName, NewPhoneNumber = newPhoneNumber}, cancellationToken);
                })
