...
public static class ReservationEndpoints
{
    ...
    private static void MapEndpoints(RouteGroupBuilder g)
    {
        ...
        g.MapGet(
            ...
            async (Guid id, int? pageSize, int? pageNumber, [FromServices] ISender sender, CancellationToken cancellationToken)=>
            {
                var result = await sender.Send(new GetReservationsByRoomIdQuery(){RoomId = id, PageNumber = pageNumber, PageSize = pageSize}, cancellationToken);
        ...
        g.MapPut(
                "",
                async (string? description, DateTime? startTime, DateTime? endTime, [FromServices] ISender sender, CancellationToken cancellationToken) =>
                {
                    await sender.Send(new ChangeReservationCommand(){NewDescription = description, NewEndReservationTime = endTime, NewStartReservationTime = startTime}, cancellationToken);
                })
            .WithSummary("Изменить бронирование");
        
        g.MapPost(
            "",
            async (Guid roomId, string? description, DateTime startTime, DateTime endTime, string userID, [FromServices] ISender sender, CancellationToken cancellationToken)=>
            {
                var result = await sender.Send(new AddReservationCommand(){RoomId = roomId, EndReservation = endTime, StartReservation = startTime, Description = description, OrganizerId = userID}, cancellationToken);
